...
# all credits to https://github.com/alvaniss/privacy-dots
...
dots=()
mic_dot="$(dot "$mic" "#a6adc8")"; [[ -n "$mic_dot" ]] && dots+=("$mic_dot")
cam_dot="$(dot "$cam" "#9399b2")"; [[ -n "$cam_dot" ]] && dots+=("$cam_dot")
loc_dot="$(dot "$loc" "#7f849c")"; [[ -n "$loc_dot" ]] && dots+=("$loc_dot")
scr_dot="$(dot "$scr" "#6c7086")"; [[ -n "$scr_dot" ]] && dots+=("$scr_dot")
text="${dots[*]}"
