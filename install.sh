...
step "1/10 — Core (Hyprland + Wayland)"
...
step "4/10 — UI (Waybar + Rofi + TUI tools)"
...
# ── نسخ إضافات zsh المدمجة من الـ dotfiles ──
...
[ -f "$ZSHRC" ] && cp "$ZSHRC" "${ZSHRC}.bak_$(date +%Y%m%d_%H%M%S)" && \
    info "Backed up existing .zshrc → ${ZSHRC}.bak_*"

cat > "$ZSHRC" << 'ZSHRC_EOF'
# ══════════════════════════════
#   .zshrc — Hyprland Dotfiles
# ══════════════════════════════
...
ok ".zshrc written → $ZSHRC"

# ══════════════════════════════════════════════════════════
#  9. Deploy Dotfiles
# ══════════════════════════════════════════════════════════

step "9/10 — Deploying Dotfiles"
...
cp -r "$DOTFILES_DIR/.config/"* "$CONFIG_DIR/" 2>/dev/null || {
    warn "Some files had permission issues, retrying with sudo..."
...
for cmd in launch-or-focus launch-tui launch-or-focus-tui launch-audio waybar-toggle-clock; do
...
# ══════════════════════════════════════════════════════════
#  10. System Services & Final Config
# ══════════════════════════════════════════════════════════

step "10/10 — System Services & Final Config"
...
# ── اكتشاف اسم لوحة المفاتيح ──
info "Detecting keyboard name for language indicator..."
KB_NAME=$(hyprctl devices -j 2>/dev/null | \
    python3 -c "
import sys,json
...
if [[ -n "$KB_NAME" ]]; then
    ...
    warn "Could not detect keyboard — update 'keyboard-name' in waybar/config.jsonc after login"
fi
...
echo -e "${DIM}  ┌─────────────────────────────────────────────────────────┐${NC}"
echo -e "${DIM}  │${NC}  ${CYN}zsh-syntax-highlighting${NC}      → تلوين الأوامر"
echo -e "${DIM}  │${NC}  ${CYN}zsh-autosuggestions${NC}          → اقتراحات تلقائية"
echo -e "${DIM}  │${NC}  ${CYN}zsh-history-substring-search${NC} → بحث في السجل"
echo -e "${DIM}  └─────────────────────────────────────────────────────────┘${NC}"
...
[[ "${answer,,}" != "n" ]] && sudo reboot || echo -e "\n${GRN}  Done! Run 'Hyprland' to start. Your shell is now zsh.${NC}\n"
